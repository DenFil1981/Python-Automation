...
@allure.epic("Data_types registrtion form")
@allure.severity("severity_level='normal")
@allure.title("Заполнение формы")
@allure.description("Заполнение формы, ввод данных и проверка валидации данных")
@allure.feature('Тест 1')


def test_assertion(chrome_browser): 
    with allure.step("Заходим на сервис по ссылке и заполняем поля и проверяем результат"):  
